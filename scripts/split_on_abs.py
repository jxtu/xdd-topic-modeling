...
abs_json_file = "xdd-covid-19-abs.json"


def sample_article_with_abs(in_json_file, in_np_file, in_abs_file) -> None:
    ...
    with open(DATA_DIR.joinpath(f"abs_{in_json_file}"), "w", encoding="utf-8") as f:
        json.dump(sampled_json_objs, f, indent=4)
    np.save(str(DATA_DIR.joinpath(f"abs_{in_np_file}")), np.array(sampled_np_objs))

    print(f"finished dumping instances with abstract!")


if __name__ == '__main__':
    sample_article_with_abs(json_file_name, np_d2v_file_name, abs_json_file)
